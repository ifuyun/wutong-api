  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    idParams.idInBody && idParams.idInBody.forEach((key) => {
      ...
      if (typeof bodyIds === 'string') {
        ...
        bodyIds.forEach((id) => {
          if (typeof id !== 'string') {
            throw new BadRequestException(format(Message.PARAM_INVALID, id), ResponseCode.ILLEGAL_PARAMS);
          }
          id = id.trim();
          if (id) {
            ids.push(id);
        ...
        throw new BadRequestException(format(Message.PARAM_INVALID, bodyIds), ResponseCode.ILLEGAL_PARAMS);
      }
    });
    ids.forEach((id) => {
